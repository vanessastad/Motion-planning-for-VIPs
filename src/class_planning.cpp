...
vector<vector<int>> class_planning::cornerHarris_fun(){ 
    ...
    for( int j = 0; j < dst_norm.rows ; j++ ){ 
        for( int i = 0; i < dst_norm.cols; i++ ){
            if( (int) dst_norm.at<float>(j,i) > thresh ){   // Detected Corner if over threshold 
    ...
    for(int i = 0; i < n; i++){
        if(c[i][0] != -1 && c[i][1] != -1){
            for(int j = i+1; j < n; j++){
                ...
                if(d < tol){   // Two corners are too close
                    flag = false;
                    j = n;
                }
            }
...
void class_planning::poly2node(){
    ...
    for(int i = 0; i < rows; i++){
        for(int j = 0; j < cols; j++){
            ...
            if(label != 0){   // If label = 0 there is an obstacle, otherwise update
    ...
    for(int i = 0; i < n_nodes; i++){
        ...
        if(indSort != -1){
            mySort[indSort] = i;
            if(indSort > maxInd)    maxInd = indSort;
        }else{
    ...
    for(i = 0; i < n_nodes; i++){
        ...
        for(j = 0; j < neighbor[i].size(); j++){            
            ...
            for(h = 0; h <= tol; h++){
                ...
                // Left proximity
                if(xp1 == xm2 - h){ // h is a tolerance interval
                    nx = xp1; 
                    vertical = 1;
                    ny = calculateOverlap(yp1,yp2,ym1,ym2,&l); 

                    if((nx-tol) > 0 && (nx+tol) < cols){ // It's inside the image 
                        f1 = false;
                        f2 = false;
...
void class_planning::adjacencies(){
    ...
    for(int i = 0; i < n_poly; i++){
        ...
        for(g = 1; g <= startDist; g++){
            if((cx-lx-distMaxX1) <= 1){ // Outside left side
                distMaxX1 = distMaxX1-1;
            }
            if((cy-ly-distMaxY1) <= 1){ // Outside upper side
                distMaxY1 = distMaxY1-1;
            }
            if((cx+lx+distMaxX2) >= cols-1){ // Outside right side
                distMaxX2 = distMaxX2-1;
            }
            if((cy+ly+distMaxY2) >= rows-1){ // Outside bottom side
...
yLine:  
            ...
            if(obs > 0 && obs < 3){
                if(fixed4 == 0) indNeighbor = clast.at<int>(fixed1,fixed3-1) -1;
                else            indNeighbor = clast.at<int>(fixed1,fixed4-1) -1;
                indEnglobing = inglobed(indNeighbor, toDelete, englobingCell);
                if(indEnglobing != -1)  indNeighbor = englobingCell[indEnglobing];
...
deleteLineY:
            if(isComparableCell(indNeighbor,index,caseRemerge)){
                ...
                if(caseRemerge == 1) goto secondTestY;
            }
            goto noDeleteLine;
...
secondTestX:
            ...
            if(obs > 0 && obs < 3){
                if(fixed4 == 0) indNeighbor = clast.at<int>(fixed3+1,fixed1) -1;
                else            indNeighbor = clast.at<int>(fixed4+1,fixed1) -1;
                indEnglobing = inglobed(indNeighbor, toDelete, englobingCell);
                if(indEnglobing != -1)  indNeighbor = englobingCell[indEnglobing];
                if(indNeighbor != -1 && indNeighbor != index){
                    caseRemerge = 4; 
    ...
    for(int i = 0; i < n_nodes; i++){
        ...
        for(j = lMin; j <= lMax-lMin; j++){
            for(int theta = 0; theta <= sampling; theta++){
                ...
                if(x > cols || x < 1 || y > rows || y < 1){ // The point is outside the image
                    cost = abs(uno/j);
                    goto newnode;
                }else{
                    if(int(src_gray.at<uchar>(y,x)) == 0){ // There is a boundery
                        cost = abs(uno/j);
                        goto newnode;
                    }
                }
...
void class_planning::addNode(int x, int y, int SoG){
    ...
    if(label == 0){
        if(src_gray.at<int>(y,x) == 0){
            ...
            if(label == 0) label = clast.at<int>(y-1,x-1);
            if(label == 0){
                cout << "The position (" << x << "," << y << ") correspond to a position with an obstacle." << endl;
                    block = true;
                    return ;
    ...
    if(SoG == 0) start = n_nodes-1;
    else         goals.push_back({n_nodes-1});
...
int class_planning::totalLen(vector<int> path){ // path
...
void class_planning::loadPOIs(){
    ...
    else cout << "There are not POIs for this map." << endl; 
...
void class_planning::findVertex(int x, int y, int lx, int ly, int ind, int* v){ 
    ...
    p1x = xm - err;   p1y = ym - err; // The error is deleted
    p2x = xp + err;   p2y = ym - err;
    p3x = xm - err;   p3y = yp + err;
    p4x = xp + err;   p4y = yp + err;
...
void class_planning::addPOIs(string c, int p){
    ...
    int n = newGoals.size();
    int ind;
    if(n > 1){ // There is at least a double POIs
    ...
    for(int i = 0; i < n; i++){  // Check multiPOis
...
void class_planning::drawNodes(){
    
     // Nodes detected Plotting
    
    Mat dst;
    dst = Mat::ones( src.size(), CV_32FC1 )*255;
    // Each node is represented with a circle 
    for( int j = 0; j < nodes.size() ; j++ ){ 
        if(nodes[j].flag != 2)
            circle(src, Point(nodes[j].cx, nodes[j].cy), 2,  Scalar(0), 2, 8, 0);
        else
            circle(src, Point(nodes[j].cx, nodes[j].cy), 8,  Scalar(0), 2, 8, 0);
    }
    imshow( source_window, src);
}


void class_planning::drawPath(vector<int> p){ 
    ...
    for(int i = 0; i < p.size()-1; i++){
        ...
        pt2.x = nodes[p[i+1]].cx;
        pt2.y = nodes[p[i+1]].cy;
        
        arrowedLine(src, pt1, pt2, (200, 200, 0), 5);
