...
// Node structure
struct my_node
{ // DA RIGUARDARE RISPETTO A FINAL_NODE
...
class class_planning : public nav_core::BaseGlobalPlanner
{

public:
    char *name_map; // Location variable fot the image we want to load
    ...
    float scale; // If provides the scale to convert [px]->[m]
    int scala;
    ...
    vector<my_node> nodes_final; // TROVARE COME CONVERTIRE IL PERCORSO FINAL IN COORDINATE

    class_planning(); // Initializes the planner attributes with default values

    class_planning(std::string name, costmap_2d::Costmap2DROS *costmap_ros); // It is used to initialize the costmap, that is the map that will be used for planning (costmap_ros), and the name of the planner (name).
